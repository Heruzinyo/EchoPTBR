...
init -1500 python in _console:
    ...
    class DebugConsole(object):
        ...
        def start(self):
            ...
            if self.first_time:
                message += __("Pressione <esc> para sair do console. Digite help para obter ajuda.\n")
                self.first_time = False

            if self.can_renpy():
                message += __("script Ren'Py habilitado.")
            else:
                message += __("script Ren'Py desabilitado.")
...
init -1500 python in _console:
    ...
    @command(_("help: mostra essa ajuda"))
    def help(l):
        ...
        if console.can_renpy():
            rv += __(" <renpy script statement>: executa statement\n")

        rv += __(" <python expression ou statement>: executa expression ou statement")
    ...
    @command(_("clear: limpa o histórico do console"))
    def clear(l):
        console.history[:] = [ ]

    @command(_("exit: sai do console"))
    def exit(l):
    ...
    @command(_("load <slot>: carrega o jogo de um arquivo de salvamento"))
    def load(l):
        name = l.rest().strip()

        if not name:
            raise Exception("O nome do arquivo de salvamento não pode estar vazio")

        try:
            renpy.load(name)
        finally:
            console.history[-1].result = "Carregando arquivo de salvamento {!r}.".format(name)


    @command(_("save <slot>: salva o jogo em um arquivo de salvamento"))
    def save(l):
        name = l.rest().strip()
        ...
        return "Arquivo de salvamento {!r} salvo.".format(name)

    @command(_("reload: recarrega o jogo, atualizando os scripts"))
    def reload(l):
        store._reload_game()
    ...
    @command(_("watch <expression>: monitorara uma expressão python"))
    def watch(l):
        expr = l.rest()
        expr.strip()
    ...
    @command(_("unwatch <expression>: para de monitorar uma expressão python"))
    def unwatch(l):
        expr = l.rest()
        expr.strip()
    ...
    @command(_("unwatchall: para de monitorar todas expressões"))
    def unwatchall(l):
        traced_expressions[:] = [ ]
    ...
    @command(_("jump <label>: vai para uma label"))
    def jump(l):
        label = l.name()

        if not console.can_renpy():
            raise Exception("Script Ren'Py não está habilitado. Não executando ação.")

        if not renpy.has_label(label):
            raise Exception("Label %s não foi encontrado." % label)

        renpy.pop_call()
        renpy.jump(label)

    @command(_("short: Reduz a representação de objetos no console (padrão)."))
    def short(l):
        persistent._console_short = True


    @command(_("long: Exibe a representação completa dos objetos no console."))
    def long(l):
